...
public class Battlecard implements Comparable<Battlecard> {
    ...
    public static class SwagComparator implements Comparator<Battlecard> {

        @Override
        public int compare(Battlecard first, Battlecard second) {
            int compare = Double.compare(second.getSwag(), first.getSwag());
            return compare != 0 ? compare : Integer.compare(first.getId(), second.getId());
        }
    }
}